...
function ProjectCard({
  ...
  return (
    <button
      ...
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      className="no-drag w-full text-left rounded-xl px-3.5 py-3 transition-all"
      style={{
        background: hovered ? 'rgba(201,168,76,0.06)' : 'rgb(var(--ov) / 0.025)',
        border: hovered ? '1px solid rgba(201,168,76,0.2)' : '1px solid rgb(var(--ov) / 0.06)',
      }}
    >
      <div className="flex items-center gap-2.5 mb-1.5">
        <svg width="13" height="13" viewBox="0 0 16 16" fill={hovered ? '#c9a84c' : 'rgb(var(--ov) / 0.3)'} className="shrink-0 transition-colors">
        ...
        <span
          ...
          style={{ color: hovered ? '#c9a84c' : 'var(--text)' }}
        >
          {caseItem.name}
        </span>
      </div>
      <div className="flex items-center gap-3 ml-[25px]">
        <span className="text-[10px]" style={{ color: 'rgb(var(--ov) / 0.35)' }}>
...
function KnowledgeFileItem({
  ...
  return (
    <div
      ...
      style={{
        background: hovered ? 'rgb(var(--ov) / 0.04)' : 'transparent',
      }}
    >
      <svg width="10" height="10" viewBox="0 0 16 16" fill="rgba(201,168,76,0.5)" className="shrink-0">
...
export default function Sidebar({
  ...
  const inProject = currentCaseId !== null && currentCase !== undefined

  // Sessions for current view
  const visibleSessions = searchQuery.trim()
    ...
    : inProject
      ...
      : sessions.filter((s) => !s.caseId)

  const groups = groupSessions(visibleSessions)

  // Project workspace knowledge files
  const projectFiles = inProject ? files.filter((f) => f.caseId === currentCaseId) : []

  // ── Project Name Editing ───────────────────────────────────────
  function startProjectNameEdit(): void {
    if (!currentCase) return
    setProjectEditName(currentCase.name)
    setEditingProjectName(true)
    projectNameCommittedRef.current = false
    setTimeout(() => projectNameInputRef.current?.select(), 0)
  }

  function commitProjectNameEdit(): void {
    if (projectNameCommittedRef.current) return
    projectNameCommittedRef.current = true
    setEditingProjectName(false)
    const trimmed = projectEditName.trim()
    if (trimmed && currentCaseId && trimmed !== currentCase?.name) {
      onRenameCase(currentCaseId, trimmed)
    }
  }
        ...
        <>
          <div className="px-2.5 pb-3">
            <button
              ...
              title="New chat"
              aria-label="New chat"
              className="no-drag flex w-full items-center justify-center rounded-xl py-2.5 transition-all"
              style={{
        ...
        <>
          ...
          <div className="px-3 pb-2">
            <button
              ...
              className="no-drag flex items-center gap-1.5 text-[11px] font-medium mb-2 transition-colors"
            ...
            <div className="flex items-center justify-between gap-2">
              {editingProjectName ? (
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') { e.preventDefault(); commitProjectNameEdit() }
                    if (e.key === 'Escape') { projectNameCommittedRef.current = true; setEditingProjectName(false) }
                  }}
                  autoFocus
                ...
                <h2
                  className="text-[15px] font-bold truncate cursor-pointer flex-1"
                  style={{ color: 'var(--text)' }}
                  onDoubleClick={startProjectNameEdit}
              ...
              <button
                onClick={() => onDeleteCase(currentCaseId)}
          ...
          <div className="px-3 pb-2">
            ...
            <button
              onClick={onAddFiles}
              disabled={isLoading}
              className="no-drag flex w-full items-center justify-center gap-2 rounded-lg px-3 py-2 text-[11px] font-medium transition-all disabled:opacity-40"
              style={{
          ...
          <div className="px-3 pb-2">
            <button
              ...
              className="no-drag flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-[12px] font-medium transition-all"
              style={{
                background: 'rgb(var(--ov) / 0.04)',
                border: '1px solid rgb(var(--ov) / 0.07)',
                color: 'rgb(var(--ov) / 0.45)',
